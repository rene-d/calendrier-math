...
def rendertex(engine, string, packages, temp_dir, block):
    ...
    try:
        ...
        logging.warning(
            "'%s' has warnings during compilation. See %s/%s", string, temp_dir, name
        )
...
def render(
    ...
    for equation, start, end, block in equations:
        ...
        if branch:
            try:
                svg = check_output(
                    ["git", "show", "%s:%s" % (branch, svg_path.replace("\\", "/"))]
                ).decode("utf-8")
            except Exception:
        ...
        try:
            if svg and not rerender:
                ...
                offset = float(
                    xml.attrib["{https://github.com/leegao/readme2tex/}offset"]
                )
        ...
        if not block:
            ...
            viewBox = [
                float(a) for a in attributes["viewBox"].split()
            ]  # min-x, min-y, width, height
            baseline_offset = viewBox[-1] - (y - viewBox[1])
            newViewBox = list(viewBox)

            newViewBox[0] = min(
                list(float(next.attrib["x"]) for next in uses if next.attrib["x"] != x)
                or [float(x)]
            )
    ...
    if has_changes:
        if not branch or branch == old_branch:
            ...
            try:
                ...
                if stashed:
                    logging.fatal(
                        "You have stashed changes on "
                        + old_branch
                        + ", make sure you unstash them there."
                    )
                raise e
    ...
    if nocdn:
        ...
        svg_url = (
            "https://cdn.jsdelivr.net/gh/{user}/{project}@{branch}/{svgdir}/{name}.svg"
        )
    ...
    for equation, start, end, block in equations:
        ...
        url = svg_url.format(
            user=user, project=project, branch=branch, svgdir=svgdir, name=name
        )
        tail = []
        if bustcache:
    ...
    if htmlize:
        try:
            ...
            logging.error(
                "Cannot render markdown, make sure that the markdown package is installed."
            )
            return
